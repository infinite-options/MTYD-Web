...
class EditPlan extends React.Component {
  ...
  changePlans = (meals, deliveries) => {
    if(this.props.hasOwnProperty('plans')){
    ...
    let object = {
      ...
      customer_lat: this.state.latitude.toString(),
      customer_long: this.state.longitude.toString(),
      driver_tip: newUpdatedPlan.payment_summary.driver_tip
    };
    console.log("(make_purchase) object (string): ", JSON.stringify(object));
    console.log("(make_purchase) object: ", object);
    ...
    axios
      ...
      .then((res) => {
        ...
        // 0907 NEW STATE UPDATE
        newUpdatedPlan.meals = meals;
        newUpdatedPlan.deliveries = deliveries;

        console.log("old payment_summary (string): ", JSON.stringify(newUpdatedPlan.payment_summary));
        console.log("old payment_summary: ", newUpdatedPlan.payment_summary);
        console.log("new payment_summary: ", res.data);

        // newUpdatedPlan.payment_summary.ambassador_discount = res.data.ambassador_discount.toFixed(2);
        // newUpdatedPlan.payment_summary.base_amount = res.data.new_meal_charge.toFixed(2);
        newUpdatedPlan.payment_summary = {
          ...
          discount_rate: mealPlan.delivery_discount,
  ...
  loadSubscriptions = (subscriptions, discounts, setDefault) => {
    ...
    subscriptions.forEach((sub, index) => {
      ...
      let nextBillingAmount =
        ...
        parsedDiscount * 0.01 * sub.subtotal -
        sub.ambassador_code;
      console.log("sub before NBA: ", sub);
      // let nextBillingAmount =
      // (sub.subtotal 
      // -sub.amount_discount)
      ...
      let payment_summary = {
        ...
        discount_rate: parsedDiscount,
        ambassador_discount: sub.ambassador_code.toFixed(2),
        subtotal: (nextBillingAmount + sub.ambassador_code).toFixed(2),
        total: nextBillingAmount.toFixed(2),
      };
  ...
  componentDidUpdate(prevProps, prevState) {
    console.log("(CUD) previous props: ", prevProps);
    console.log("(CUD) current props: ", this.props);
    if (
    ...
    if(prevProps.selectedPlan !== this.props.selectedPlan){
      console.log("(CUD) previous selectedPlan: ", prevProps.selectedPlan);
      console.log("(CUD) current selectedPlan: ", this.props.selectedPlan);
      this.changePlans(
      ...
      mealButtons.push(
        <div
          ...
          onClick={() => {
            ...
            console.log("(SSM) clicked: ", sub.meals, " ; ", sub.deliveries);
            console.log("(SSM) plans: ", this.props.plans);
